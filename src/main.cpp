...
/** Fees smaller than this (in umota) are considered zero fee (for relaying and mining)
...
void RecalculateZMOTASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMOTA supply
...
bool RecalculateMOTASupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateMOTASupply(Params().Zerocoin_StartHeight());
    }
