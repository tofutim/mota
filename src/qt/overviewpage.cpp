...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeMotaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMotaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeMotaAmount = strAnonymizeMotaAmount.remove(strAnonymizeMotaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMotaAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeMotaAmount * COIN) nMaxToAnonymize = nAnonymizeMotaAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeMotaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeMotaAmount));
        strAnonymizeMotaAmount = strAnonymizeMotaAmount.remove(strAnonymizeMotaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMotaAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
